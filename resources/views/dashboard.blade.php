...
@if((Auth::user()->user_type == 'admin'))
    @php
        $customers = DB::table('users')->where('user_type', 'customer')->where('is_merchant', 0)->get();
        $company_profit = 0;
        $company_today_profit = 0;
        foreach ($customers as $customer){
            $result = DB::table('order_details')->where('user_id', $customer->id)->where('purchased_by', 'user')->sum('profit');
            $company_profit += $result;
            $today_result = DB::table('order_details')->where('user_id', $customer->id)->where('purchased_by', 'user')->whereDate('created_at', DB::raw('CURDATE()'))->sum('profit');
            $company_today_profit += $today_result;
        }
    @endphp
    <div class="row">
        <div class="col-md-4">
            <div class="panel">
                <div class="panel-body text-center dash-widget dash-widget-left">
                    ...
                    <br>
                    <br>
                    <p class="text-normal text-main">{{__('Customer Sells Income')}}</p>
                    <p class="text-semibold text-3x text-main">100 %</p>
                    <br>
                    <br>
                </div>
            </div>
        ...
        <div class="col-md-4">
            <div class="panel">
                <div class="panel-body text-center dash-widget">
                    <br>
                    <p class="text-normal text-main">{{__('Customer Sells Total Profit')}}</p>
                    <p class="text-semibold text-3x text-main">{{ number_format($company_profit, 2) }}</p>
                    <br>
                    <a href="#" class="btn-link">{{__('View Report')}} <i class="fa fa-long-arrow-right"></i></a>
                    <br>
                    <br>
                </div>
            </div>
        ...
        <div class="col-md-4">
            <div class="panel">
                <div class="panel-body text-center dash-widget">
                    <br>
                    <p class="text-normal text-main">{{__('Customer Sells Today Profit')}}</p>
                    <p class="text-semibold text-3x text-main">{{ number_format($company_today_profit, 2) }}</p>
                    <br>
                    <a href="#" class="btn-link">{{__('View Report')}} <i class="fa fa-long-arrow-right"></i></a>
                    <br>
                    <br>
...
@if((Auth::user()->user_type == 'admin'))
    @php
          ...
          //$todaydepositProfit = DB::table('deposits')->where('user_id', $user->id)->where('created_at', carbon\Carbon::today())->sum('deposit_amount');

            $merchants = DB::table('users')->where('is_merchant', 1)->get();
            $todaydepositProfit = 0;
            foreach ($merchants as $merchant){
                $today_result = DB::table('order_details')->where('user_id', $merchant->id)->where('purchased_by', 'merchant')->whereDate('created_at', DB::raw('CURDATE()'))->sum('profit');
                $todaydepositProfit += $today_result * (37.5/100);
        ...
        <div class="col-md-4">
            <div class="panel">
                <div class="panel-body text-center dash-widget dash-widget-left" style="background: #294F75;">
                    ...
                    <br>
                    <br>
                    <p class="text-normal text-main" style="color: #fff;">{{__('Merchant Sells Income')}}</p>
                    <p class="text-semibold text-3x text-main" style="color: #fff;">37.5 %</p>
                    <br>
                    <br>
                </div>
            </div>
        ...
        <div class="col-md-4">
            <div class="panel">
                <div class="panel-body text-center dash-widget">
                    ...
                    <p class="text-semibold text-3x text-main">{{ number_format($depositProfit, 2) }}</p>
                    <br>
                    <a href="#" class="btn-link">{{__('View Report')}} <i class="fa fa-long-arrow-right"></i></a>
                    <br>
                    <br>
        ...
        <div class="col-md-4">
            <div class="panel">
                <div class="panel-body text-center dash-widget">
                    ...
                    <p class="text-semibold text-3x text-main">{{ number_format($todaydepositProfit, 2) }}</p>
                    <br>
                    <a href="#" class="btn-link">{{__('View Report')}} <i class="fa fa-long-arrow-right"></i></a>
                    <br>
                    <br>
...
@if(Auth::user()->user_type == 'admin')
    @php
        ...
        $merchants = DB::table('users')->where('user_type', 'customer')->where('is_merchant', 1)->count();
        $customers = DB::table('users')->where('user_type', 'customer')->where('is_merchant', 0)->count();
    @endphp
    <div class="row">
        <div class="col-md-6">
            <div class="panel">
                <div class="panel-body text-center dash-widget dash-widget-left">
                    ...
                    <div class="pad-ver mar-top text-main">
                        <i class="demo-pli-add-user icon-4x"></i>
                    </div>
                    <br>
                    <p class="text-lg text-main">{{__('Total Users')}}: <span class="text-bold">{{ $users }}</span></p>
                    <p class="text-lg text-main">{{__('Total BDO')}}: <span class="text-bold">{{ $bdos }}</span></p>
                    <p class="text-lg text-main">{{__('Total Distributor')}}: <span class="text-bold">{{ $distributors }}</span></p>
                    <p class="text-lg text-main">{{__('Total Merchant')}}: <span class="text-bold">{{ $merchants }}</span></p>
                    <p class="text-lg text-main">{{__('Total Customer')}}: <span class="text-bold">{{ $customers }}</span></p>
                    <br>
                </div>
            </div>
        </div>
                ...
                <div class="col-sm-6">
                    <div class="panel">
                        <div class="pad-top text-center dash-widget">
                            ...
                            <p class="text-semibold text-3x text-main">{{ $customers }}</p>
                            <a href="{{ route('all-customer.index') }}" class="btn btn-primary mar-top btn-block top-border-radius-no">{{__('View All Customers')}}</a>
                        </div>
                    </div>
                    <div class="panel">
                        <div class="pad-top text-center dash-widget">
                            <p class="text-normal text-main">{{__('Total Distributor')}}</p>
                            <p class="text-semibold text-3x text-main">{{ $distributors }}</p>
                            <a href="{{ route('all-distributor.index') }}" class="btn btn-primary mar-top btn-block top-border-radius-no">{{__('View All Distributors')}}</a>
                        </div>
                    </div>
                </div>
                <div class="col-sm-6">
                    <div class="panel">
                        <div class="pad-top text-center dash-widget">
                            <p class="text-normal text-main">{{__('Total Merchant')}}</p>
                            <p class="text-semibold text-3x text-main">{{ $merchants }}</p>
                            <a href="{{ route('all-merchant.index') }}" class="btn btn-primary mar-top btn-block top-border-radius-no">{{__('View All Merchants')}}</a>
                        </div>
                    </div>
                    <div class="panel">
                        <div class="pad-top text-center dash-widget">
                            <p class="text-normal text-main">{{__('Total BDO')}}</p>
                            <p class="text-semibold text-3x text-main">{{ $bdos }}</p>
                            <a href="{{ route('all-bdo.index') }}" class="btn btn-primary mar-top btn-block top-border-radius-no">{{__('View All BDOS')}}</a>
                        </div>
                    </div>
                </div>
            </div>
