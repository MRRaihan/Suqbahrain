    ...
    public function dashboard()
     {
        ...
        elseif($user->user_type == 'customer'){
            if ($user->status == 1) {
                ...
                //Today Merchant Profit

                  $merchant_profit = DB::table('order_details')->where('user_id', $user->id)->where('purchased_by', 'merchant')->whereDate('created_at',Carbon::today())->sum('profit');
                  $merchant_today_profit = $merchant_profit * (50/100);
                ...
                if($bankinfo == null){
                    $bankinfo = DB::table('bank_infos')->select('id', 'ac_holder', 'ac_no', 'bank_name', 'iban_number')->where('user_id', $user->id)->where('status', 'secondary')->first();
                }
                ...
                if( $lastwith != null ){
                    $lastwithdraw = $lastwith->created_at;
                } else {
                    $lastwithdraw = $user->created_at;
                ...
                return view('frontend.customer.dashboard', compact('merchant_today_profit', 'availbleProfit', 'depositPoint', 'bankinfo', 'withdrawamount', 'lastwithdraw'));
            } else {
                Auth::logout();
                return view('frontend.user_login');
            }
